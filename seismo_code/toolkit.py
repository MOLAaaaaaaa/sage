...
def run_gmt(
    ...
    ----------
    script : str
        完整的 GMT bash 脚本内容（推荐使用 GMT6 modern mode）。
    ...
    GMT6 modern mode 示例（传入 script 参数）：
        gmt begin map_name PNG
          gmt basemap -R0/120/20/60 -JM15c -Baf
          gmt coast -Gtan -Slightblue -W0.5p
        gmt end
    """
    ...
    # 将 script 中 'gmt begin <任意名> [格式]' 替换为正确的输出名 + PNG
    if _re.search(r'gmt\s+begin', script, _re.IGNORECASE):
        ...
        script_mod = script   # GMT5 legacy：保留原始内容

    # 写脚本文件
    header = (
        f"#!/bin/bash\n"
        f"# GMT script — {title}\n"
        f"# Generated by SAGE SeismicX\n\n"
        f"cd '{outdir}'\n\n"
    )
    with open(out_sh, 'w', encoding='utf-8') as f:
        f.write(header + script_mod)
    ...
    # GMT6 在 cwd 中生成 <outname>.png
    if not os.path.exists(out_png):
        # 尝试 PostScript → PNG 转换（GMT5 遗留）
