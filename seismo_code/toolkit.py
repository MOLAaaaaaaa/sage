...
def run_gmt(
    ...
    """
    执行 GMT (Generic Mapping Tools) bash 脚本，返回输出 PNG 图像路径。
    同时将脚本保存为 .sh 文件，供下载。

    中文标题/标签自动处理：GMT 的 PostScript 引擎不支持 CJK 字符，
    run_gmt 会自动提取脚本中的中文文本、用空占位符替换，
    执行完成后再通过 matplotlib 叠加回 PNG，确保中文正确显示。

    Parameters
    ----------
    script : str
        完整的 GMT bash 脚本（GMT6 modern mode，推荐）。
        脚本里 ``gmt begin`` 后的文件名会被自动替换为 outname。
        中文标题写在 ``-BWSne+t"中文标题"`` 里即可，无需特殊处理。
    outname : str
        输出文件基名（不含扩展名），默认 "gmt_map"。
    title : str
        脚本注释标题（英文或中文均可），不影响图像。
    ...
    需要系统安装 GMT >= 6.0：
      ...
      Linux:  conda install -c conda-forge gmt  或  apt install gmt
    """
    import subprocess, shutil, re as _re

    # 检查 GMT 是否可用
    gmt_bin = shutil.which('gmt')
    if gmt_bin is None:
        raise RuntimeError(
    ...
    if _re.search(r'gmt\s+begin', script, _re.IGNORECASE):
        ...
        script_mod = script   # GMT5 legacy

    # ── 提取中文文本，替换为空占位（避免 GMT PostScript 乱码）────────────
    script_mod, cjk_texts = _extract_cjk_texts(script_mod)

    # ── 写脚本文件（保留原始中文注释，方便用户阅读）──────────────────────
    header = (
        "#!/bin/bash\n"
        f"# GMT script — {title}\n"
        "# Generated by SAGE SeismicX\n"
        "# Note: CJK titles are post-processed via matplotlib\n\n"
        f"cd '{outdir}'\n\n"
    )
    with open(out_sh, 'w', encoding='utf-8') as f:
        # 脚本文件保留用户原始版本（含中文），供下载参考
        original_header = (
            "#!/bin/bash\n"
            f"# GMT script — {title}\n"
            "# Generated by SAGE SeismicX\n\n"
        ...
        f.write(original_header + script)   # 保存原始脚本（含中文）
    os.chmod(out_sh, 0o755)

    # ── 用去中文后的版本实际执行 ──────────────────────────────────────────
    exec_script = header + script_mod
    exec_sh = os.path.join(outdir, outname + "_exec.sh")
    with open(exec_sh, 'w', encoding='utf-8') as f:
        f.write(exec_script)
    os.chmod(exec_sh, 0o755)

    proc = subprocess.run(
        ['bash', exec_sh],
        capture_output=True, text=True,
        cwd=outdir, timeout=180,
    )
    # 删除临时执行脚本
    try:
        os.unlink(exec_sh)
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"GMT 执行失败（返回码 {proc.returncode}）:\n{proc.stderr}"
        )
